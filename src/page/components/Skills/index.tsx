    ...
    <div ref={referance} className="pt-32 sm:pl-28 pl-12">
      ...
      <div className="pb-20 flex justify-start grid grid-cols-1 md:grid-cols-2 lg:pr-12 ">
        {skillsMock?.map((skill) => (
          <div
            className="flex flex-col w-full md:w-4/5 pb-4 sm:pb-10 progress-container"
            key={skill?.id}
          >
            <span className="sm:text-3xl text-xl py-4 font-[Inter] font-light text-tertiary">
              {skill?.name}
            </span>
            <Progress
              percent={skill?.prcnt}
      ...
      <div className="md:text-3xl text-lg font-normal w-full text-right -mt-12 pr-8 sm:pr-20">
